...
async function Profile(sender_psid, id, info = null) {
    ...
    let imgLink = info[10];
    if (imgLink == "") {
        imgLink = "https://cdn-icons-png.flaticon.com/512/666/666201.png";
    }
    const response = {
        attachment: {
            type: "template",
...
async function Info(sender_psid, text, page = 1) {
    if (sender_psid != "306816786589318") {
        console.log("Info: ", sender_psid, "Command: ", text);
    }
    let textSplit;
    try {
    ...
    const subj = `${textSplit.length > 2 ? textSplit.at(-2) + " " : ""
    }${textSplit.at(-1)}`
        .replace(/lí/, "lý")
        .toLowerCase();
    ...
    if (sender_psid != "306816786589318") {
        console.log("Info Found: ", name, subj, gender, page, resG.num);
    }
    if (resG.num == 0) {
        postMessenger(sender_psid, {
            text: "Không tìm thấy dữ liệu trùng khớp! Hãy kiểm tra lại cú pháp! (nhắn '!info')",
        });
        return;
    }
    if (resG.num == 1) {
    ...
    const response = {
        text: `Đã tìm thấy ${resG.num} kết quả! [Trang ${page}/${Math.ceil(
            resG.num / 10,
        )}]`,
